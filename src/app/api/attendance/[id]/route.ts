...
import { collection, getDocs, addDoc, deleteDoc,getDoc, doc, updateDoc, Timestamp ,DocumentReference} from 'firebase/firestore';
...
export async function GET(req: Request) {
  try {
    console.log("oke")
    // 🔹 Ambil query params dari request URL
    ...
    // 🔹 Tentukan rentang tanggal (default: 1 minggu terakhir)
    const today = new Date();
    const toDate = toDateParam ? new Date(toDateParam) : today;
    const fromDate = fromDateParam ? new Date(fromDateParam) : new Date(today.setDate(today.getDate() - 6));
